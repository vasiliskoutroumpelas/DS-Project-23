...
int main()
{
    ...
    root = makeBalancedTree(root);
    cout << "Entry with Date " << Date2 << " has been successfully deleted." << endl
         << endl;
...
BstNode *Insert(BstNode *root, string date, long int value)
{
    ...
    if (date_to_int(date) == date_to_int(root->date))
    {
...
void inOrderRepr(BstNode *node, vector<BstNode *> &nodes)
{
    ...
    cout << node->date << " ( " << node->count << " ) "
         << " | " << node->value << endl; // code for inorder traversal representation
...
BstNode *deleteByDate(BstNode *root, string date)
{
    ...
    if (date_to_int(date) < date_to_int(root->date))
        return deleteByDate(root->left, date);

    else if (date_to_int(date) > date_to_int(root->date))
        return deleteByDate(root->right, date);
